...
const ColumnMappingComponent = ({ selectedColumns, onBack, onSubmit }) => {
  ...
  const handleSubmitMapping = async () => {
    ...
    if (Object.keys(changedMappings).length > 0) {
      try {
        await axios.post("http://3.6.36.17:80/api/send-column-mappings/", {
...
function App() {
 ...
 const handleSubmitForAnalysis = async () => {
  ...
  try {
    ...
    const dataResponse = await axios.get("http://3.6.36.17:80/api/get-data/");
  ...
  const handleDelete = async () => {
    try {
      ...
      const deleteResponse = await axios.delete(
        "http://3.6.36.17:80/api/delete-all/"
      );
  ...
  const handleMappingSubmit = async (mappings) => {
    ...
    try {
      ...
      const uploadResponse = await axios.post(
        "http://3.6.36.17:80/api/upload-csv2/", 
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
